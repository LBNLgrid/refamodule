        ...
        for event, e in et.iterparse(source, events=('start', 'end')):
            ...
            # End event
                ...
                if e.tag == 'table':
                    ...
                    # Store the value and unit if they exist
                    odict['value'] = convert_type(e.text) if e.text else None
                    unit = e.attrib.get('units', None)  # Get the 'units' attribute
                    
                    if unit:
                        # Append the unit to the key name
                        odict[f'{e.tag}_{unit}'] = odict['value']
                    else:
                        odict[e.tag] = odict['value']
