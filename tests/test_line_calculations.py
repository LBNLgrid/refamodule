...
def test_ampacity(line1):
    result = line1.ampacity_at_environment()
...
def test_sag_at_current(line1):
    result = line1.sag_at_current(current_a=1400, initial_tension_percentage=0.2)
    assert result == approx(13.560, rel=1e-3)


def test_sag_at_temperature(line1):
    result = line1.sag_at_temperature(
...
def test_sag_at_power_and_voltage(line1):
    result = line1.sag_at_power_and_voltage(
...
def test_corona_inception_voltage(line1, structure_config_ac):
    result = line1.corona_inception_voltage(structure_config=structure_config_ac)
    assert result == approx(230.184, rel=1e-3)


def test_corona_voltage_gradient(line1, structure_config_ac):
    result = line1.corona_voltage_gradient(structure_config=structure_config_ac)
...
def test_resistive_line_losses_from_current(line1):
    result = line1.resistive_line_losses(current_a=1400, load_factor=0.6)
    assert result == approx(2.780, rel=1e-3)


def test_resistive_line_losses_with_congestion(line1):
    result = line1.resistive_line_losses_considering_congestion(
...
def test_corona_discharge_losses_ac(line1, structure_config_ac):
    result = line1.corona_discharge_losses(
...
def test_congestion_from_power(line1):
    result = line1.congestion(power_mw=700, voltage_kv=230)
...
def test_overall_performance_current_only(line1):
    ...
    assert "ampacity_a" in result
    assert "sag_m" in result
    assert result["ampacity_a"] == approx(1398.261, rel=1e-3)
...
def test_overall_performance_power_and_voltage(line1):
    ...
    assert "ampacity_a" in result
    assert "sag_m" in result
    assert "congestion_mw" in result


def test_overall_performance_with_losses(line1):
    result = line1.overall_technical_performance(current_a=1500, load_factor=0.6)
    assert "resistive_losses_mwh_per_m" in result
    assert result["resistive_losses_mwh_per_m"] == approx(3.390, rel=1e-3)
...
def test_overall_performance_with_corona(line1, structure_config_ac):
    ...
    assert "corona_inception_voltage_kv" in result
    assert "corona_losses_mwh_per_m" in result
    assert result["corona_inception_voltage_kv"] == approx(230.184, rel=1e-3)
